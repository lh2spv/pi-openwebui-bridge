...
- リクエストごとに pi を1プロセス起動（コールドスタート）。

## RPC 常駐版（任意・高速）

`pi_bridge.py` は毎回 pi を起動しますが、**[`pi_bridge_rpc.py`](pi_bridge_rpc.py)** は
`pi --mode rpc` を常駐させて使い回し、プロセス起動コストを消します（実測で短い応答が
約 2 倍速、0.8s → 0.4s）。使い方は同じ（環境変数も共通）で、**既定ポートは 8766**。

```powershell
$env:PI_BRIDGE_MODELS = "my-agent=--provider <provider> --model <model-id>"
python pi_bridge_rpc.py
```

- モデルごとに 1 常駐プロセス、起動時にウォームアップ。
- リクエスト毎に `new_session` でリセット（OWUI は毎回フル履歴を送るため）。
- 画像は RPC の `images`(base64) で直接渡す。同時リクエストはプロセス単位の Lock で直列化
  （各 PC 1 ユーザ想定）。プロセス死亡時は自動再起動。
- トレードオフ: 速い代わりに長寿命プロセスを持つ。高同時実行が要るならプロセスプール化が必要。
